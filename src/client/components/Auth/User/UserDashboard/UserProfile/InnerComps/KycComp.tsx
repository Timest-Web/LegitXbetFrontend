import React from "react";
import ProfileVeriReuse from "../ProfileReusables/ProfileVeriReuse";
import Image from "next/image";
import pdfImg from "../../../UserAssets/export.png";

const KycComp = () => {
  return (
    <div>
      <ProfileVeriReuse isInsideKycComp />
      <div className="bg-white w-[61.25rem] h-[34rem] rounded-[20px] p-12 mt-4">
        <div className="font-bold flex justify-between mb-8">
          <h2>Upload your Kyc for verification</h2>
          <h2>Verified documents would appear Here</h2>
        </div>
        <h2 className="font-bold mb-4">Document type</h2>
        <div>
          <form className="flex flex-col space-y-9">
            <div className="flex space-x-10 font-bold">
              <input className="w-[1.375rem] h-[1.375rem]" type="radio" />
              <label>Int. Passport</label>
              <input className="w-[1.375rem] h-[1.375rem]" type="radio" />
              <label>NIN</label>
              <input className="w-[1.375rem] h-[1.375rem]" type="radio" />
              <label>Voters card</label>
              <input className="w-[1.375rem] h-[1.375rem]" type="radio" />
              <label>Drivers License</label>
            </div>
            <div className="bg-[#ECEEF1] border-[#101010] border-2 border-dashed w-[18.0625rem] h-[8.6875rem] rounded-[20px] flex flex-col justify-center items-center">
              <Image src={pdfImg} alt="upload" />
              <h2>Upload pdf only</h2>
            </div>
            <button className="bg-[#101010] text-white w-[10.75rem] h-[2.375rem] p-1 rounded-[5px] ml-12">
              Send for verification
            </button>
          </form>
        </div>
        <p className="mt-12">
          NB - Only Government-issued documents would be approved
        </p>
      </div>
    </div>
  );
};

export default KycComp;
